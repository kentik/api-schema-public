...
func (x *Market) Proto_ShallowCopy(v interface{}) {
	...
	case *Market:
		x.Id = v.GetId()
		x.Name = v.GetName()
		x.GeoScope = v.GetGeoScope()
		x.GeoName = v.GetGeoName()
		x.ChildList = v.GetChildList()
		x.ParentList = v.GetParentList()
	default:
		...
		if v, ok := v.(interface{ GetGeoScope() string }); ok {
			x.GeoScope = v.GetGeoScope()
		}
		if v, ok := v.(interface{ GetGeoName() string }); ok {
			x.GeoName = v.GetGeoName()
		}
		if v, ok := v.(interface{ GetChildList() []string }); ok {
			x.ChildList = v.GetChildList()
		}
		if v, ok := v.(interface{ GetParentList() []string }); ok {
			x.ParentList = v.GetParentList()
		}
	}
}
...
func (x *Market) Proto_ShallowClone() (c *Market) {
	if x != nil {
		...
		c.Id = x.Id
		c.Name = x.Name
		c.GeoScope = x.GeoScope
		c.GeoName = x.GeoName
		c.ChildList = x.ChildList
		c.ParentList = x.ParentList
	}
	return
}
